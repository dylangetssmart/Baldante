...
if OBJECT_ID(N'dbo.Integer2Date', N'FN') is not null
	drop function dbo.Integer2Date;
go
...
Format time for UDF
*/ ------------------------------------------------------------------------------
if OBJECT_ID('dbo.FormatUDFTime', 'FN') is not null
	drop function dbo.FormatUDFTime;

go

CREATE OR ALTER FUNCTION dbo.FormatUDFTime (@InputValue VARCHAR(50))
RETURNS VARCHAR(20)
AS
BEGIN
    RETURN
    (
        REPLACE(
            REPLACE(
                LTRIM(
                    RIGHT(
                        CONVERT(VARCHAR(20), TRY_CONVERT(TIME, @InputValue), 100),
                        7
                    )
                ),
            'AM', ' AM'),
