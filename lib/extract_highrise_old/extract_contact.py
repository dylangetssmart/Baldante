...
def extract_contact_header(data, file_path):
    """ Contact Record -----------------------------------------------------------------------------
    id = data[0]['ID']
    name = data[0]['Name']
    tags = data[0]['Tags']
...
def extract_contact_info(data, contact_id, engine, file_path, progress=None):
...
def extract_contact(file_path, engine, progress=None):
    ...
    data = load_yaml(file_path, progress.console)
    if not data:
        return 

    # Contact Header
    contact_data = extract_contact_header(data, file_path)
    insert_to_sql_server(file_path, engine, 'contacts', contact_data)

    # Contact Information
    extract_contact_info(data, contact_data['id'], engine, file_path, progress)

    # Notes and Tasks
    notes, tasks = extract_notes_and_tasks(data, contact_id=contact_data['id'])
...
if __name__ == '__main__':
    ...
    extract_contact(args.input, engine)